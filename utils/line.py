...
class Line:
    ...
    def update_fits(self, left_fit, right_fit):
        ...
        if self.first_frame_processed:
            ...
            if left_error < self.mse_tolerance:
                self.left_fit = 0.7 * self.left_fit + 0.3 * left_fit
            if right_error < self.mse_tolerance:
                self.right_fit = 0.7 * self.right_fit + 0.3 * right_fit
        else:
    ...
    def update_curvature(self, fit):
        ...
        if self.first_frame_processed:
            ...
            self.curvature = 0.7 * self.curvature + 0.3 * (((1 + y1 * y1) ** (1.5)) / np.absolute(y2))
